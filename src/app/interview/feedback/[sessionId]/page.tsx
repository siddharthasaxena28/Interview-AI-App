...
export default async function FeedbackPage({
      ...
      <div className="min-h-screen bg-slate-50">
        ...
        <div className="flex items-center justify-center min-h-[80vh]">
          <div className="text-center">
            ...
            <p className="text-gray-700 font-semibold">Generating your feedback report…</p>
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900 px-6 pt-10 pb-20 text-center">
        <div className="max-w-4xl mx-auto">
          <div className="flex flex-wrap items-center justify-center gap-2 mb-3">
            <span className="text-slate-300 text-sm font-medium">{s.company}</span>
          ...
          {interviewDate && (
            <p className="text-slate-400 text-sm mt-2">{interviewDate}</p>
          )}
        </div>
      </div>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 pb-16 space-y-5 -mt-12 relative">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8">
          ...
          <div className="mt-6 pt-5 border-t border-gray-100 flex flex-wrap items-center justify-center gap-2">
            <span className="text-xs text-gray-400">
              Industry average for {getRoundLabel(s.round_type as RoundType)}:
            </span>
            <span className="text-xs font-semibold text-gray-600">{benchmarkAvg}%</span>
        ...
        <div className="grid md:grid-cols-2 gap-5">
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="p-5">
              ...
              <div className="space-y-3">
                {strengths.map((str, i) => (
                  <div key={i} className="flex gap-3 p-3.5 rounded-xl bg-green-50 border border-green-100">
                    <div className="w-5 h-5 rounded-full bg-green-200 text-green-700 text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">
                      {i + 1}
                    </div>
                    <div>
                      <div className="font-semibold text-green-900 text-sm mb-0.5">{str.title}</div>
                      {str.example && (
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="p-5">
              ...
              <div className="space-y-3">
                {gaps.map((g, i) => (
                  <div key={i} className="flex gap-3 p-3.5 rounded-xl bg-amber-50 border border-amber-100">
                    ...
                    <div>
                      <div className="font-semibold text-amber-900 text-sm mb-0.5">{g.title}</div>
                      {g.example && (
          ...
          <div className="p-5 sm:p-6">
            ...
            {commDimensions ? (
                    ...
                    <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                      <div
                        className={`h-2 rounded-full ${
                          dim.score >= 80 ? 'bg-green-500' : dim.score >= 60 ? 'bg-amber-500' : 'bg-red-500'
                        }`}
              ...
              <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                <div
                  className={`h-2 rounded-full ${
                    r.communication_score >= 80 ? 'bg-green-500' : r.communication_score >= 60 ? 'bg-amber-500' : 'bg-red-500'
                  }`}
        ...
        {topicData.length > 1 && (
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="p-5 sm:p-6">
              <h2 className="font-semibold text-gray-900 mb-4 flex items-center gap-2">
                <TrendingUp className="w-4 h-4 text-indigo-500" /> Performance by Topic
              </h2>
              <div className="space-y-3">
                    ...
                    <div key={tag}>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className={`text-xs font-semibold ${
                          avg >= 4 ? 'text-green-600' : avg >= 3 ? 'text-amber-600' : 'text-red-600'
                        }`}>
                          {avg.toFixed(1)}/5
                        </span>
                      </div>
                      <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                        <div
                          className={`h-2 rounded-full ${
                            avg >= 4 ? 'bg-green-500' : avg >= 3 ? 'bg-amber-500' : 'bg-red-500'
                          }`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Link
            ...
            className="flex items-center justify-center gap-2 bg-blue-600 text-white px-6 py-3 rounded-xl font-semibold text-sm hover:bg-blue-700 transition-colors flex-1 shadow-sm"
          >
          ...
          <a
            ...
            className="flex items-center justify-center gap-2 border border-gray-200 bg-white text-gray-700 px-6 py-3 rounded-xl font-semibold text-sm hover:bg-gray-50 transition-colors shadow-sm"
          >
